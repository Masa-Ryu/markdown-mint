...
let activePage;
const errors = [];
const report = { sourceCommit: process.env.GITHUB_SHA, capture: "production webview in browser harness; asynchronous simulated host transport", actions: [], errors };
const theme = {
...
try {
  ...
  const context = await browser.newContext({ viewport: { width: 1180, height: 760 }, deviceScaleFactor: 1, colorScheme: "dark", permissions: ["clipboard-read", "clipboard-write"] });
  ...
  assert.deepEqual(await page.locator(".ProseMirror .selectedCell").allTextContents(), ["Visual editing", "Ready", "Alice", "Table controls", "In review", "Sam"]);
  await page.mouse.move(4, 4);
  await frame(1.6);
  ...
  await page.waitForTimeout(300);
  await frame(1.2);
  report.actions.push("Real mouse drag selected the expected 2 x 3 cells; real Ctrl+C/Ctrl+V pasted the matrix; one Ctrl+Z restored the source");
