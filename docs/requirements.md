...
| R04         | Rectangular table selection and matrix clipboard | ProseMirror tables use a rectangular selection and matrix clipboard path, with Markdown round trips in the core tests. Outside an existing table, valid multi-cell internal, TSV, or HTML-table clipboard data becomes a header-first table; TSV display values take priority over spreadsheet HTML, while one-cell and ordinary multiline text remain native paste. A shared validator rejects malformed or over-10,000-cell matrices without truncation. In the 0.0.3 evidence, a real 2×3 rectangle copy/paste preserved its dimensions and one Undo request restored the prior table in the browser harness; native VS Code resource Undo/Redo was verified independently.                                                                                                                                                                                 |
...
| Q04 profile and resource limits        | Markdown sources are capped at two million UTF-16 code units, clipboard matrices are capped at 10,000 cells with whole-paste rejection, operation ids and resource URLs are bounded, and relative local images resolve through scoped webview resources.                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
## 0.0.53 spreadsheet table paste
...
- `tableClipboard.ts` owns the bounded matrix parser/validator, clipboard
  ...
  fallback, and header-first table-node construction. The Insert Table dialog
  keeps its 20-column/50-row UI limits; clipboard matrices use the separate
  10,000-cell limit and are rejected as a whole when oversized.
- In the Rich Editor's normal document flow, multi-cell internal clipboard,
  TSV, and HTML-table payloads insert one table transaction. Paragraph text is
  split around the inserted block, list and blockquote containers remain
  intact, and a `BlockBoundarySelection` is handled at its insertion point.
  TSV display values win over spreadsheet HTML, styles/formulas/metadata are
  not imported, and the first row is normalized to header cells.
- One-cell payloads, ordinary multiline text, CommonMark, Code/raw/source
  surfaces, native textareas/inputs, Alert bodies, and rendered/source editing
  surfaces retain plain-text/native paste behavior. Existing in-table matrix
  paste, expansion, CellSelection replacement, copy, cut, and malformed
  internal-cell fallback remain covered by the webview suite.
- Regression coverage checks Unicode, empty cells, quoted TSV, CRLF and inner
  newlines, literal quotes/pipes/HTML punctuation, HTML `rowspan`/`colspan`,
  TSV-versus-HTML priority, internal rich cells, one-cell rejection, boundary
  insertion, nested containers, CommonMark/Code exclusions, and the 10,000 /
  10,001-cell boundary. Actual Ctrl/Cmd clipboard gestures and operating
  system IME candidate behavior remain manual checks rather than claims from
  synthetic clipboard events.
