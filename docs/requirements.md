...
| R04         | Rectangular table selection and matrix clipboard | ProseMirror tables use a rectangular selection and matrix clipboard path, with Markdown round trips in the core tests. Outside an existing table, valid multi-cell internal, TSV, or HTML-table clipboard data becomes a header-first table; TSV display values take priority over spreadsheet HTML, while one-cell and ordinary multiline text remain native paste. Clipboard flavors are evaluated lazily in internal → TSV → HTML → CellSelection 1×1 order. A shared validator rejects malformed or over-10,000-cell matrices without truncation; malformed or oversized table HTML cannot fall through to the native HTML parser, and the final serialized Markdown source is checked before a spreadsheet transaction commits. In the 0.0.3 evidence, a real 2×3 rectangle copy/paste preserved its dimensions and one Undo request restored the prior table in the browser harness; native VS Code resource Undo/Redo was verified independently. |
...
- `tableClipboard.ts` owns the bounded matrix parser/validator, clipboard
  ...
  fallback, and header-first table-node construction. The TSV parser stops as
  soon as prospective rectangular padding would exceed 10,000 cells. The
  Insert Table dialog keeps its 20-column/50-row UI limits; clipboard matrices
  use the separate 10,000-cell limit and are rejected as a whole when
  oversized.
...
- Regression coverage checks Unicode, empty cells, quoted TSV, CRLF and inner
  ...
  lazy TSV-versus-HTML priority, internal rich cells, one-cell rejection,
  boundary insertion, nested containers, malformed/oversized HTML safeguards,
  CommonMark/Code exclusions, final serialized-source rejection, and the
